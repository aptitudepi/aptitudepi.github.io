...
function renderRadar(host, data) {
  ...
  RADAR_KEYS.forEach((k, i) => {
    ...
    const lx = cx + (radius + 24) * ca;
    const ly = cy + (radius + 24) * Math.sin(angle(i)) + 4;
    const vx = cx + (radius + 40) * ca;
    const vy = cy + (radius + 40) * Math.sin(angle(i)) + 4;
...
function markUnavailable(host, sel, msg) {
  ...
  if (el) {
    ...
    msgEl.className = 'gh-unavailable';
